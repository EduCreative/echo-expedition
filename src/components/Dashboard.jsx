...
function UserProfileSummary({ user }) {
  ...
  // FIX: Use default values and Number() casting to prevent NaN if user.level or user.xp are not yet loaded.
...
export default function Dashboard() {
  ...
  const { totalLessonsCompleted, totalXpEarned } = useMemo(() => {
    ...
    // FIX: Use default values and Number() casting to prevent NaN calculations.
    const userLevel = Number(user.level || 1);
    const userXp = Number(user.xp || 0);
    const xpFromLevels = (userLevel - 1) * XP_PER_LEVEL;
    const totalXp = xpFromLevels + userXp;
    ...
    <div className="dashboard">
      ...
      <div className="dashboard-stats">
        <div className="stat-item streak">
          ...
          <span className="stat-value">{dailyStreak.count} Day{dailyStreak.count !== 1 ? 's' : ''}</span>
