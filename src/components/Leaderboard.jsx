...
async function fetchLeaderboardData() {
    ...
    // Fetch both collections concurrently for performance.
    const [usersSnapshot, progressSnapshot] = await Promise.all([
        getDocs(usersCollection),
        getDocs(progressCollection)
    ]);

    // Process users, safely extracting only the necessary primitive data.
    const usersData = {};
    usersSnapshot.forEach(doc => {
        const data = doc.data();
        // This prevents non-serializable objects from being processed.
        usersData[doc.id] = { name: String(data.name || 'Anonymous') };
    });

    // Process progress, safely extracting only the necessary primitive data.
    const progressData = {};
    progressSnapshot.forEach(doc => {
        const data = doc.data();
        progressData[doc.id] = {
            pronunciationRaceHighScore: Number(data.pronunciationRaceHighScore || 0),
            listeningDrillHighScore: Number(data.listeningDrillHighScore || 0),
        };
    });

    // Combine the sanitized data into a single array of plain objects.
    const combinedData = Object.keys(usersData).map(uid => {
        ...
        return {
            id: uid,
            name: user.name,
            pronunciationRaceHighScore: progress.pronunciationRaceHighScore,
            listeningDrillHighScore: progress.listeningDrillHighScore,
        };
    });
