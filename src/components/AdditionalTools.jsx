...
import { startCustomLesson, startPronunciationRace, startListeningDrill, goToLeaderboard, startPracticeMode, startVocabularyBuilder } from '../lib/actions';
        ...
        return (
            <div className="pronunciation-race-widget">
                <div className="widget-header">
                    <h3><span className="icon">timer</span> Select Difficulty</h3>
                    <p>Choose a level for your Pronunciation Race.</p>
                </div>
                <div className="widget-level-selection">
                    <button className="button" onClick={() => handleStart('easy')}>Easy</button>
                    <button className="button" onClick={() => handleStart('medium')}>Medium</button>
                    <button className="button" onClick={() => handleStart('hard')}>Hard</button>
                </div>
                 <button className="button" onClick={() => setShowLevels(false)} style={{marginTop: '16px'}}>
    ...
    if (showLevels) {
        return (
            <div className="listening-drill-widget">
                <div className="widget-header">
                    <h3><span className="icon">hearing</span> Select Difficulty</h3>
                    <p>Choose a level for your Echo Drill session.</p>
                </div>
                <div className="widget-level-selection">
                    <button className="button" onClick={() => handleStart('easy')}>Easy</button>
                    <button className="button" onClick={() => handleStart('medium')}>Medium</button>
...
function VocabularyBuilderWidget() {
    const { isProcessing, isOnline, vocabularyBuilderHighScore, isAiEnabled } = useStore();
    return (
        <div className="vocabulary-builder-widget">
            <div className="widget-header">
                <h3><span className="icon">spellcheck</span> Vocabulary Builder</h3>
                <p>See an image, then spell and pronounce the word for AI feedback.</p>
            </div>
            <div className="widget-score">
                High Score: <strong>✨ {vocabularyBuilderHighScore}</strong>
            </div>
            <button
                className="button primary"
                onClick={startVocabularyBuilder}
                disabled={isProcessing || !isOnline || !isAiEnabled}
                title={!isAiEnabled ? 'AI features are required' : (!isOnline ? 'Internet connection required' : '')}
            >
                Start Building
            </button>
        </div>
