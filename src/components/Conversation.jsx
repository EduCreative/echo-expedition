...
const getAvatarForScenario = (title) => {
  if (!title) return 'auto_awesome';
  const lowerTitle = title.toLowerCase();
  if (lowerTitle.includes('food')) return 'restaurant';
  if (lowerTitle.includes('directions')) return 'explore';
  if (lowerTitle.includes('interview')) return 'work';
  if (lowerTitle.includes('doctor')) return 'local_hospital';
  if (lowerTitle.includes('plans')) return 'celebration';
  return 'auto_awesome';
};
...
export default function Conversation() {
  ...
  const avatarIcon = getAvatarForScenario(scenarioTitle);
