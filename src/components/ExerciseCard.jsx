...
import { startRecording, stopRecording, speakText, changePrompt, submitForFeedback, discardRecording, recordAgain } from '../lib/actions';
...
const getAvatarForLesson = (lesson) => {
  if (!lesson) return 'smart_toy';
  if (lesson.lessonType === 'boss_battle') return 'shield_person';
  
  const title = lesson.title.toLowerCase();
  if (title.includes('cafe')) return 'local_cafe';
  if (title.includes('hotel')) return 'hotel';

  // Fallback for other roleplays
  return 'smart_toy';
};
...
export default function ExerciseCard() {
  ...
  const textToTranscribe = useMemo(() => {
    ...
    if (currentLesson.lessonType === 'dialogue_comprehension' || currentLesson.lessonType === 'comprehension') {
        return currentPrompt.question || '';
    }
    return currentPrompt.correctText || currentPrompt.text || '';
  }, [currentPrompt, currentLesson.lessonType]);
  ...
  const avatarIcon = getAvatarForLesson(currentLesson);
  ...
  return (
    <div className="exercise-card">
      <section className="prompt-section">
        ...
        {lessonType === 'dialogue_comprehension' && (
          <div className="chat-container">
            {currentLesson.dialogue.map((msg, index) => (
                <div key={index} className='chat-bubble ai'>
                    <AIAvatar icon={'record_voice_over'} />
                    <p>{msg.speaker}: {msg.line}</p>
                </div>
            ))}
          </div>
        )}
          ...
          <div className="chat-container">
            {currentLesson.chatHistory.map((msg, index) => (
              <div key={index} className={c('chat-bubble', msg.role)}>
                {msg.role === 'ai' && <AIAvatar icon={avatarIcon} />}
        ...
        <div className="response-area">
           {pendingRecording && !hasFeedback ? (
            <div className="recording-preview">
             ...
             <>
                {(!isAiEnabled || !isOnline) && !isInteractive && !hasFeedback && (
        ...
        <section className="feedback-section">
            ...
            {currentPrompt.feedback === 'OFFLINE_RECORDING' ? (
                <div className="offline-comparison-section">
                  ...
                  <button className="button" onClick={recordAgain} style={{ marginTop: '24px', display: 'block', margin: 'auto' }}>
                    <span className="icon">replay</span> Try Again
                  </button>
                </div>
            ) : (
              <>
                ...
                <div className="correct-pronunciation-player">
                  <button className="button" onClick={handleListenClick}>
                    <span className="icon">model_training</span> Listen to Correct Pronunciation
                  </button>
                </div>
                <div className="transcript" style={{marginTop: '16px'}}>
                    <h3>Your Answer</h3>
                ...
                {(currentLesson.lessonType === 'sentence_ordering' || currentLesson.lessonType === 'fill_in_the_blank' || currentLesson.lessonType === 'grammar_drill') && currentPrompt.score < 100 && (
