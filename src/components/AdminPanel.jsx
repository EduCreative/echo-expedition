...
function AdminAnalytics({ users }) {
  ...
  const getCommonOptions = (title) => ({
    ...
    plugins: {
      ...
      tooltip: {
        enabled: true,
        backgroundColor: isDark ? 'rgba(48, 49, 52, 0.9)' : 'rgba(248, 249, 250, 0.9)',
...
export default function AdminPanel() {
  ...
  // Memoize the user fetching function to prevent re-creation on every render.
  // It will only be updated when the network status changes.
  ...
  // Fetch users on component mount and whenever the network status changes.
  useEffect(() => {
    refreshUsers();
  }, [refreshUsers]);
