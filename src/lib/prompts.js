...
export const levels = {
  ...
  b2: {
    ...
    lessons: [
      ...
      { title: 'Dialogue: At the Airport', icon: 'flight', type: 'dialogue_comprehension', difficulty: 'Hard' },
  ...
  g1: {
    ...
    emoji: '🏋️',
    lessons: [
      { title: 'Present Tense Verbs', icon: 'spellcheck', difficulty: 'Easy', type: 'grammar_drill' },
      { title: 'Past Tense Verbs', icon: 'spellcheck', difficulty: 'Easy', type: 'grammar_drill' },
      { title: 'Prepositions', icon: 'spellcheck', difficulty: 'Medium', type: 'grammar_drill' },
      { title: 'Articles', icon: 'spellcheck', difficulty: 'Medium', type: 'grammar_drill' },
    ],
  },
};
...
export const getVocabularyBuilderPrompt = (targetWord, userSpelling, userTranscript) => {
  const prompt = `An English learner was shown an image of a "${targetWord}".
They spelled the word as: "${userSpelling}".
They also pronounced the word, which was transcribed as: "${userTranscript}". The audio is attached.

Analyze their response:
1.  **Spelling Check:** Is the user's spelling ("${userSpelling}") exactly correct for the target word ("${targetWord}")? Ignore case.
2.  **Pronunciation Score:** Provide a numerical score from 0 to 100 for pronunciation, based SOLELY on the audio. Evaluate clarity and accuracy compared to a standard native speaker's pronunciation of "${targetWord}".
3.  **Feedback:** Provide concise, helpful feedback (2-3 sentences) covering both spelling (if incorrect) and pronunciation. Address the student directly as "you". If both are perfect, give a short encouraging message.

Return ONLY a JSON object with three keys: "spellingCorrect" (boolean), "pronunciationScore" (integer), and "feedback" (string).`;

  const schema = {
    type: Type.OBJECT,
    properties: {
      spellingCorrect: { type: Type.BOOLEAN, description: "True if the user's spelling is correct (case-insensitive)." },
      pronunciationScore: { type: Type.INTEGER, description: "A numerical score from 0 to 100 for pronunciation accuracy." },
      feedback: { type: Type.STRING, description: "The written feedback for the user." },
    },
    required: ["spellingCorrect", "pronunciationScore", "feedback"],
  };
...
export const getFeedbackPrompt = (lessonType, context, userTranscript) => {
  ...
  switch (lessonType) {
    ...
    case 'fill_in_the_blank':
      ...
      break;
    case 'grammar_drill':
      promptIntro = `This is a grammar drill. An English learner was shown a sentence with a blank, often with a verb in parentheses: "${context.promptText}". The correct, grammatically complete sentence is: "${context.correctText}". They responded with the attached audio, which was transcribed as: "${userTranscript}". The feedback should focus primarily on whether they used the correct grammatical form to complete the sentence.`;
      break;
    case 'dialogue_comprehension':
      const dialogueText = context.dialogue.map(d => `${d.speaker}: ${d.line}`).join('\n');
      promptIntro = `An English learner read a dialogue:\n${dialogueText}\n\nThey were then asked the question: "${context.question}". They responded with the attached audio, which was transcribed as: "${userTranscript}". The expected answer is similar to: "${context.correctAnswer}". Evaluate their reading comprehension based on their answer, focusing on accuracy and relevance, not just grammar.`;
      break;
    case 'comprehension':
