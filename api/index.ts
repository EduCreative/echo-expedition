...
// Set JSON limits for large audio base64 objects
...
app.get("/api/health", (req, res) => {
  res.json({ status: "ok", provider: "vercel-serverless" });
});
...
app.post("/api/gemini/generateContent", async (req, res) => {
  try {
    ...
    console.error("Vercel Serverless generateContent Error:", error);
...
app.post("/api/gemini/generateImages", async (req, res) => {
  try {
    ...
    console.error("Vercel Serverless generateImages Error:", error);
    res.status(500).json({ error: error instanceof Error ? error.message : String(error) });
  }
});
